...
This add-on is part of the [naked-head/homeassistant-addons](https://github.com/naked-head/homeassistant-addons)
collection, forked from [griffinmartin/ha-app-bambu-studio-api](https://github.com/griffinmartin/ha-app-bambu-stdio-api).

---

## Architecture

- **amd64 only.** Bambu Lab does not publish an official ARM64 AppImage, so this add-on cannot be installed on aarch64 Home Assistant hosts. On those hosts run the sidecar on a separate x86_64 machine and point Bambuddy at it over your LAN. See the
...
`3001` matches the Bambuddy wiki's default for `bambu-studio-api`. Change the host port only if 3001 collides with something else on your Home Assistant host. Bambuddy's virtual-printer feature already reserves host ports 3000 and 3002, so **don't move this add-on to either of those**.

---

## Pointing Bambuddy at this add-on

1. Make sure this add-on is **started** and the **Log** tab shows the wrapper listening on port 3000.
...
which is accessible via the **File Editor** add-on. Data survives restarts, add-on updates, and reinstalls (as long as you opt to keep add-on data when uninstalling).
...
The Bambu Studio AppImage version is pinned at **image build time** via the `BAMBU_VERSION` build-arg in the add-on's Dockerfile. 
Bambu Studio version updates are handled **automatically**: a weekly GitHub Actions workflow checks for new [BambuStudio releases](https://github.com/bambulab/BambuStudio/releases) and opens a Pull Request against [naked-head/homeassistant-addons](https://github.com/naked-head/homeassistant-addons) when a newer version is available. Once the PR is merged, a new add-on image is built and published automatically, and Home Assistant will offer the update through the normal add-on update mechanism.
To upgrade Bambu Studio you have to update the add-on itself (a new release that bumps the build-arg and the add-on `version`). 
...
Expected on first install. The add-on Dockerfile downloads the Bambu Studio AppImage (~220 MB) and builds the Node wrapper from source. Subsequent restarts and add-on updates reuse the cached image.
...
- That Bambuddy and this add-on can reach each other (use the HA host's LAN IP, not `localhost`, if Bambuddy runs in its own container on a different Docker network).
...
In HA you can use the **SSH & Web Terminal** add-on plus `docker exec -it addon_<slug>_bambu_studio_api bash`.
