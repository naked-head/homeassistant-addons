...
for f in scenarios/*.json; do
  ...
  if [ "$ok" = 1 ]; then echo "  ✔ HTTP 8000 up"
  else echo "  ✘ HTTP 8000 DOWN"; echo "$L" | tail -30; FAIL=1; fi
 ...
 grep -qiE 'traceback|unbound variable|Failed to get addon config' <<<"$L" && {
  ...
  case "$s" in
    full)
      grep -q 'Trusted frame origins: http://192.168.50.2:8123,https://example.com' <<<"$L" \
        || { echo "  ✘ trusted_frame_origins not applied"; FAIL=1; }
      grep -q 'External roots: /share:/media' <<<"$L" \
        || { echo "  ✘ external roots not applied"; FAIL=1; }
      grep -q 'Home Assistant URL: http://192.168.50.2:8123' <<<"$L" \
